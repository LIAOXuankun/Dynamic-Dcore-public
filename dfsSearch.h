...
#ifndef DYNAMIC_DCORE_DFSSEARCH_H
#define DYNAMIC_DCORE_DFSSEARCH_H

#include "Common.h"

class DfsSearch final{
private:
    ...
    uint32_t hIndex (const vector<uint32_t> &input_vector);
    void lMaxRemove (vector<uint32_t> &m_d, vector<uint32_t> &in_degree, vector<bool> &vbDeleted, uint32_t cur_node,
...
public:
    DfsSearch(vector<pair<uint32_t, uint32_t> > & edges, const  vector<vector<pair<::uint32_t,::uint32_t>>> & old_d_core_decomposition);   //onetime initialization
    DfsSearch(vector<pair<uint32_t, uint32_t> > & edges, const  vector<vector<pair<::uint32_t,::uint32_t>>> & old_d_core_decomposition, vector<pair<uint32_t, uint32_t>> & modified_edges);   //multiple initialization
    DfsSearch(const DfsSearch&) = delete;
    DfsSearch& operator=(const DfsSearch&) = delete;

    //vector<::uint32_t> in_coreness;


    void maintainKmax(const vector<pair<uint32_t, uint32_t>> & modified_edges, bool is_insert, const uint32_t &M);  // maintain the kmax value of vertices, is_insert = true means insertion, otherwise deletion
    void maintainKlist(const vector<pair<uint32_t, uint32_t>> & modified_edges, bool is_insert, const uint32_t &M,
                       bool k0core_pruning, bool use_hindex, const int &lmax_number_of_threads,
                       const bool &reuse_pruning, const bool &skip_pruning);  // based on the updated kmax value, maintain the k-lists, i.e., l_{max}(v,k) value
...
#endif //DYNAMIC_DCORE_DFSSEARCH_H
